...
from app.agent import DocumentDetails
...
async def test_agent_stream_query(mock_client_class, agent_app: AgentEngineApp) -> None:
    """
    ...
    for event in events:
        ...
        if validated_event.output or (
            validated_event.content is not None
            and validated_event.content.parts
            and any(part.text for part in validated_event.content.parts)
        ):
