...
echo "▸ 2/7 Firestore database (native mode)…"
gcloud firestore databases create --location="${REGION}" || true

echo "▸ 3/7 Composite index (status + next_check_at) for the scheduler query…"
gcloud firestore indexes composite create \
  --collection-group=documents \
  --field-config field-path=status,order=ascending \
  --field-config field-path=next_check_at,order=ascending || true

echo "▸ 4/7 Service account + least-privilege IAM (Design Doc §6.2)…"
...
echo "▸ 5/7 Document AI processor (Document OCR)…"
...
echo "▸ 6/7 Secrets → Secret Manager (never in the repo)…"
create_secret() {
...
create_secret TELEGRAM_WEBHOOK_SECRET   "${TELEGRAM_WEBHOOK_SECRET}"
...
gcloud run deploy "${SERVICE}" \
  ...
  --set-env-vars "GCP_PROJECT_ID=${PROJECT_ID},GCP_LOCATION=${DOCAI_LOCATION}" \
  --set-secrets "TELEGRAM_BOT_TOKEN=TELEGRAM_BOT_TOKEN:latest,GEMINI_API_KEY=GEMINI_API_KEY:latest,DOCAI_PROCESSOR_ID=DOCAI_PROCESSOR_ID:latest,SCHEDULER_TOKEN=SCHEDULER_TOKEN:latest,TELEGRAM_WEBHOOK_SECRET=TELEGRAM_WEBHOOK_SECRET:latest"
# max-instances is pinned to 1: pending-confirmation state and webhook dedupe
# live in process memory (app/main.py). Move them to Firestore before scaling.

SERVICE_URL=$(gcloud run services describe "${SERVICE}" --region "${REGION}" --format='value(status.url)')
TOKEN_HASH=$(printf '%s' "${TELEGRAM_BOT_TOKEN}" | sha256sum | cut -c1-32)

echo "▸ Registering Telegram webhook (with secret_token header auth)…"
curl -s "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/setWebhook?url=${SERVICE_URL}/webhook/${TOKEN_HASH}&secret_token=${TELEGRAM_WEBHOOK_SECRET}"
echo

echo "▸ Registering the bot command menu (/expiring, /packet, /help)…"
